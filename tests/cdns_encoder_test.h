...
namespace CDNS {
    static const std::string filename("test.out");

    void cmp(std::string& result) {
        struct stat buff;
    ...
    TEST(CdnsEncoderTest, CECTest) {
        CdnsEncoder* enc = new CdnsEncoder(filename, CborOutputCompression::NO_COMPRESSION);
        struct stat buff;
    ...
    TEST(CdnsEncoderTest, CERotateTest) {
        ...
        struct stat buff;
        std::ifstream file("test2.out.cdns");
        std::string str((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
        EXPECT_EQ(result2, str);
